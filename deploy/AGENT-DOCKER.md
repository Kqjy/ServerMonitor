...
```
docker build -f deploy/agent.Dockerfile -t registry.example.com/servermonitor-agent:0.1.6 .
docker push registry.example.com/servermonitor-agent:0.1.6
```
...
SM_AGENT_IMAGE=registry.example.com/servermonitor-agent:0.1.6
```
...
```
docker compose -f deploy/docker-compose.agent.yml --env-file .env.agent up -d
...
- **Upgrades**: auto-upgrade is disabled in containers (`SM_AUTO_UPGRADE=false`). To upgrade, build and push a new image tag, bump `SM_AGENT_IMAGE`, then `docker compose ... up -d`. Host-installed agents still self-upgrade normally.
