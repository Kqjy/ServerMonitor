...
```bash
docker build -f deploy/agent.Dockerfile -t registry.example.com/servermonitor-agent:0.2.3 .
docker push registry.example.com/servermonitor-agent:0.2.3
```
...
SM_AGENT_IMAGE=registry.example.com/servermonitor-agent:0.2.3
```
