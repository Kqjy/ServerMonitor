...
```bash
docker build -f deploy/agent.Dockerfile -t registry.example.com/servermonitor-agent:0.4.7 .
docker push registry.example.com/servermonitor-agent:0.4.7
```
...
SM_AGENT_IMAGE=registry.example.com/servermonitor-agent:0.4.7
```
