...
```bash
docker build -f deploy/agent.Dockerfile -t registry.example.com/servermonitor-agent:0.4.5 .
docker push registry.example.com/servermonitor-agent:0.4.5
```
...
SM_AGENT_IMAGE=registry.example.com/servermonitor-agent:0.4.5
```
